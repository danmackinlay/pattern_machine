...
PSMCCore {
	...
	*makeSynthDef {|numChannels|
		^SynthDef.new(this.synthName(numChannels), { |inbus=0, outbus=0, in=0, out=0|
			Out.ar(outbus+out, In.ar(inbus+in, numChannels));
		});
	}
}
...
PSMCMix : PSMCCore {
	...
	*makeSynthDef {|numChannels|
		^SynthDef.new(this.synthName(numChannels), { |inbus=0, outbus=0, in=0, out=0|
			Out.ar(outbus+out, Mix.new(In.ar(inbus+in, numChannels)));
		});
	}
}