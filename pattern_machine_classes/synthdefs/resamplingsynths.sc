...
PSResamplingSynthDefs {
	...
	*loadSynthDefs {
		...
		SynthDef.new(\ps_buf_delay_play__1x2, {
			...
			Out.ar(outbus+out, Pan2.ar(sig, pan));
		}).add;
		
		SynthDef.new(\ps_echette__1x2,
			{|inbus=0, outbus=0, in=0, out=0, deltime=0.1, ringtime=1, amp=1, pan=0|
				var env, sig;
				sig = In.ar(inbus+in, 1) * EnvGen.kr(
				...
				Out.ar(outbus+out, sig);
				}, [\ir, \ir, \ir, \ir, \ir, \ir]
		).add;
		SynthDef.new(\ps_echette_colored__1x2,
			{|inbus=0, outbus=0, in=0, out=0, deltime=0.1, ringtime=1, amp=1, freq=440, color=0.5, pan=0|
