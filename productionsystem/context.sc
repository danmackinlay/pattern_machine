/*
a PSContext generalises partial application to Events. One may compose PSContexts to one another to produce a transformed
context, and to Events to transform events.

Note that an alternative approach that might work nicer could be to subclass EnvironmentRedirect from JITLib, which dispatches to 
a wrapped environment but cna set defaults.
*/
PSContext : IdentityDictionary {
	applyTo {|that|
		//<> composition per default operates right to left, but contexts are more
		//intuitively applied left to right, and we don't want to have to patch Event to make this work
		// (Which we weould have to do, since that which is composed *with() gets to define the method.)
		// so we make a new method which is basically <>, but backwards.
		// Additionally, we assume that the values in this guy are functions and that
		// we will compose by partial application instead of by replacing values
		// Be careful with the order when using this guy.
		// a b c d E should apply transforms a
